...
// that composed them, and the atoms underneath both.
...
export function WikiTabs({ counts }: { counts: { pages: number; days: number; runs: number; atoms: number } }) {
  ...
  const tabs = [
    ...
    { href: "/wiki/time", label: "Over time", note: `${counts.days} days · ${counts.runs} runs`, active: pathname.startsWith("/wiki/time") },
    { href: "/wiki/knowledge", label: "Knowledge", note: `${counts.atoms.toLocaleString("en-US")} atoms`, active: pathname.startsWith("/wiki/knowledge") },
  ];
