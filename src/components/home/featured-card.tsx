...
// `Card` with the link as its element; the card owns no padding (the inner
// block does).
...
export function FeaturedCard({ thread, priority }: { thread: FeaturedThread; priority?: boolean }) {
  return (
    <Card
      ...
      size="none"
      variant="interactive"
      className="group relative w-full cursor-pointer text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
    >
      <Link href={`/featured/${thread.slug}`}>
