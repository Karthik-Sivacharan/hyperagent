...
//
// AND IT CAN BE NARROWED TO ONE CARD. A name in the conversation means the
// agent, so `focusAgent` pulses everything that agent holds. The composer's
// agent bar means one RUN, and a run is one task, so `focusTask` pulses that
// card alone. The pulse carries both ids either way — the agent, because the
// panel's announcement and its scroll target are written in terms of a person
// ("3 tasks for EvalBot"), and the task only when the ask was that precise.
// A card matches on the task when there is one and on the assignee when there
// is not, which is the whole difference between the two calls.
//
...
/** Which cards should pulse, and which ask this is. `taskId` narrows it from
    everything the agent holds to the one card that was asked for. */
export type AgentPulse = { agentId: string; taskId?: string; token: number };
...
export interface RoomTrackerValue {
  ...
  /** Show the board and pulse exactly one card. No-ops for a task this room
      does not hold, which is what an ephemeral run from the composer is. */
  ...
  /** The room's live work as the composer's agent bar wants it (§6.1). */
  agentRuns: AgentRun[];
}
...
export function RoomTrackerProvider({
  ...
  const focusTask = React.useCallback(
    (taskId: string) => {
      // The task is resolved here rather than in the caller, because the
      // panel's readout is written in terms of the agent and would otherwise
      // have to look it up a second time. Nothing found means the run has no
      // card — it was started by a mention and the board does not know about
      // it yet — and the right answer to that is to stay put.
      const task = tasks.find((candidate) => candidate.id === taskId);
      if (!task) return;
      onShowTracker?.();
      setQuery("");
      setPulse((current) => ({ agentId: task.assigneeId, taskId, token: (current?.token ?? 0) + 1 }));
    },
    [onShowTracker, tasks],
  );
