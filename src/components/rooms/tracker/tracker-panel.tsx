...
export function TrackerPanel({ className }: { className?: string }) {
  ...
  // message, the commit that showed this tab at all), put the first of the
  // agent's cards on screen. `block: "nearest"` so a card already in view is
  // left where it is.
  React.useEffect(() => {
    if (!pulse) return;
    const frame = requestAnimationFrame(() => {
      const target = areaRef.current?.querySelector<HTMLElement>(
