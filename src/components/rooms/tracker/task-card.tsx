...
//
// A pulse that names a task answers from that card alone — a chip in the
// composer's agent bar is one run, and a run is one task, so lighting up the
// agent's other three cards would be answering a question nobody asked. The
// broad ask still reads the assignee, so the two calls differ by one field.
...
export function TaskCard({ task }: { task: RoomTask }) {
  ...
  // cards with the arrow keys); `data-assignee` and `data-task-id` are how the
  // panel finds the card to scroll to when a pulse arrives — the assignee's
  // first card for a broad ask, this exact one when the ask named a task.
  const marks = {
    "data-tracker-card": "",
