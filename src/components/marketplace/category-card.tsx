...
// Phase 2: the brand's interactive card (the 22px `shadow-card` tile that
// lifts on hover) with the link as the shell, the name in the heading face.
// The tile's own colours come from the listing data and stay, as cover art
// does, painted inline over the card's default fill (docs/brand/design.md
// §5, §6).
//
...
export function CategoryCard({ category }: { category: MarketplaceCategory }) {
  ...
  return (
    <Card
      ...
      size="none"
      variant="interactive"
      className="relative aspect-[5/3] justify-end p-5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
    >
      <a href={`/marketplace/c/${category.slug}`} style={{ backgroundColor: category.background, color: category.color }}>
        <div
          aria-hidden="true"
