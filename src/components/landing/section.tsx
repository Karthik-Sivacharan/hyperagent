...
// ground. `scroll-mt-16` keeps an anchored section clear of the sticky header.
export function LandingSection({
  ...
  return (
    <section id={copy.id} aria-labelledby={headingId} className="scroll-mt-16 px-4 sm:px-6">
      <div className={cn("mx-auto max-w-6xl border-t border-border-subtle py-20 md:py-32", className)}>
        <div className="flex max-w-content flex-col gap-4">
          <Overline asChild>
            <p>{copy.eyebrow}</p>
          </Overline>
          <h2 id={headingId} className="text-3xl text-foreground md:text-4xl">
            <TwoToneText heading={copy.heading} />
          </h2>
          {copy.intro ? <p className="text-lg text-muted-foreground">{copy.intro}</p> : null}
        </div>
