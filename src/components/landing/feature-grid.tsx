...
// Every list on the page keeps an explicit `role`: the preflight strips the
// markers, and some screen readers drop list semantics with them.
    ...
    >
      {items.map((item, index) => (
        <li key={item.title} className="flex flex-col gap-5">
          {item.asset ? (
            <AssetSlot asset={item.asset} className="aspect-4/3" />
          ) : null}
