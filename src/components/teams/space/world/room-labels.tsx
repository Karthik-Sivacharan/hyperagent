// Each room's name, set like a small plate on the wall just right of its door
...
/** Gap between a label and the door edge it starts from, in CSS px. */
const GAP = 6;
      ...
      {SIGNS.map((s) => {
        ...
        return (
          <span
            key={s.room}
            aria-hidden="true"
            className="pointer-events-none absolute z-1 flex h-5 -translate-y-1/2 select-none items-center whitespace-nowrap rounded-sm bg-background/85 px-1.5 font-medium text-muted-foreground text-xs shadow-edge"
            style={{ top: faceMid, left: s.x * TILE * scale + GAP }}
          >
