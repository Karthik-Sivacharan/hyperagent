...
Branch `feat/teams-space`, worktree `.claude/worktrees/teams-space`, dev server `http://localhost:3002` (running; do not start another). A fourth view, `?view=space`, labelled **Office** in the view switch, beside Board, List and Org chart. Same data (`useFleet()`), same agent sheet. A 2D top-down pixel office in the spirit of a 2D virtual-office product: departments are rooms, every agent and every present person is a character, and you can walk around, drag anyone and hover anyone. v1 is a prototype: no backend, no persistence (positions reset on reload), light verification.

Research behind it lives outside the repo, in the git-ignored `docs/research/`
folder and the session scratchpad the builders worked from: one file on the
interaction model and chrome of the 2D virtual-office products (measured
read-only), and one survey of engines, reusable code and asset licences. Those
files name the products; this plan describes the patterns only.

## 1. Decisions

- **No game engine.** The virtual-office products themselves render the map to one canvas and lay name tags and bubbles over it as DOM. We do the same, one step further: floors, carpets and walls paint once into a `<canvas>`; furniture and characters are absolutely positioned DOM sprites, y-sorted by `z-index`; tags, bubbles and cards are DOM chrome at screen scale. No dependency is added, nothing needs `ssr: false`, and every character is a real `Button` with an accessible name, so keyboard and screen readers work as they do on the other views. If the scene ever needs zoom or hundreds of sprites, PixiJS 8 behind a `ui/` primitive is the fallback.
- **Art is licence-clean.** An MIT pixel sprite pack for floors, walls, carpets, furniture and the human characters; optionally a CC-BY 4.0 office furniture set (credit required) for the modern pieces. Agents are **robots**: palette-swapped from that pack's character sheets by a script (grey metal skin, a visor or antenna, body in the department hue). Every pack, author, licence and URL is named once, in `public/space/CREDITS.md`, which also lists the packs and the products that were deliberately not used. Nothing at all comes from the virtual-office products: no code, no art.
- **Code borrowed with its MIT header:** the sprite pack's `findPath` / `isWalkable` (BFS), its walk/idle/type timing, its wall bitmask auto-tiling and its floor colorize. Not its renderer. The header names the pack and its author; see `public/space/CREDITS.md`.
...
- The week 37 growth review (RUN-224, scheduled, Atlas with helpers Iris, Rook, Quill) is **in the meeting room**: those four sit round the table. Their own desks are empty. This is the genre's rule: a meeting room means a multi-agent run.
...
**Name tag** (the 2D-office name pill in the brand): 24px tall, fully rounded or 8px radius, `text-xs font-medium`, a small caret pointing down at the head, about 10px above it, pointer-events pass through to the character. Agent: a 20px `AgentAvatar` (the brand orb, so an agent is never mistaken for a person) and the name; a 14px state glyph after the name only when paused (`IconPlayerPause`, `text-warning`) or in error (`IconAlertTriangle`, `text-destructive`). Person: a presence mark (filled dot online, hollow ring away; neutral, not green) and the name. The tag's colours follow the ink `Tooltip`.

**Needs you bubble**: a small `bg-card shadow` bubble up and to the right of the head holding `IconProgressHelp` in `text-brand-accent`. It is the only tangerine in the view. It never merges away into a group tag, and neither does an error glyph (the lesson from those products: an error must never hide inside a merge).
